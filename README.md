...
5. Сохраняет прогресс в JSON-файл, включая тексты вопросов и ссылки на первоисточники.
...
2. Для работы системы необходим ключ Google Gemini API. Перейдите в Google AI Studio: https://aistudio.google.com/. Нажмите "Get API key" и создайте новый ключ. Используется модель gemini-2.5-flash, у которой присутствуют лимиты на запросы, что учитывается в коде. Файл с ответами создается постепенно, по мере обновления ежедневного лимита: ответы дописываются в submission_*.json, повторный запуск продолжает с места остановки.
